...
int main(int argc, char* argv[]) {
  ...
  //Genetic Algorithm
  TSPGenetic genetic(cityGraph, numberOfCities);

  auto start = chrono::high_resolution_clock::now();
  double geneticResult = genetic.computeShortestTour();
  auto end = chrono::high_resolution_clock::now();

  auto duration = chrono::duration_cast<chrono::milliseconds>(end - start).count();

  cout << "Genetic Best Tour: " << geneticResult << " Time: " << duration << "ms" << endl;
  cout << "Best Tour: ";

  for (int city : genetic.getBestTour()) {
    cout << city << " ";
  }
  cout << endl;
